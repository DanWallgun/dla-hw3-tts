...
def get_mel_v2_energy_pitch(filename):
    audio, sampling_rate = load_wav_to_torch(filename)
    if sampling_rate != _stft.sampling_rate:
        raise ValueError("{} {} SR doesn't match target {} SR".format(
            sampling_rate, _stft.sampling_rate))
    ...
    pitch, t = pw.dio(
        audio.numpy().astype(np.float64),
        hparams.sampling_rate,
        frame_period=hparams.hop_length / hparams.sampling_rate * 1000,
    )
    pitch = pw.stonemask(audio.numpy().astype(np.float64), pitch, t, hparams.sampling_rate)

    return (
        melspec.numpy().astype(np.float32),
        energy.numpy().astype(np.float32),
        pitch
    )
